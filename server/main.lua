lib.callback.register("sharky_crafting:server:CraftItem", function(source, recipe, coords)
    if exports.ox_inventory:CanCarryItem(source, recipe.item, recipe.amount) then
        ...
        sendToDiscord(_U('successful_craft'), _U('craft_format', GetPlayerName(source), recipe.item, recipe.amount, coords), 65280)
    else
        ...
        sendToDiscord(_U('failed_craft'), _U('craft_format', GetPlayerName(source), recipe.item, recipe.amount, coords), 16711680)
    end
end)


lib.callback.register("sharky_crafting:server:StartedCraft", function(source, recipe, coords)
    sendToDiscord(_U('craft_started'), _U('craft_format', GetPlayerName(source), recipe.item, recipe.amount, coords),  16776960)
end)


lib.callback.register("sharky_crafting:server:StoppedCraft", function(source, recipe, coords)
    sendToDiscord(_U('stopped_craft'), _U('craft_format', GetPlayerName(source), recipe.item, recipe.amount, coords), 16711680)
end)
